...
class AjaxController extends \yii\web\Controller {
    ...
    protected function securityLog($event, array $context = []) {
        ...
        \Yii::warning($context, 'security.import');
    }
    ...
    protected function getLoadColumnList($txtfile, $table) {
        $table = $this->validateTableName($table);
        $handle = fopen($txtfile, 'r');
        if ($handle === false) {
            throw new \RuntimeException("Can not read $txtfile");
        }
        ...
        if ($header === false) {
            throw new \RuntimeException("Can not read header from $txtfile");
        }
        ...
        foreach ($schema->columns as $name => $column) {
            $available[strtoupper($name)] = $name;
        }

        $quoted = [];
        foreach ($columns as $column) {
            if (!preg_match('/^[A-Za-z0-9_]+$/', $column)) {
                throw new \RuntimeException("Invalid column name $column in $txtfile");
            }
            $key = strtoupper($column);
            if ($key === 'CLINICALVALUE' && !isset($available[$key]) && isset($available['CLINICALVAULE'])) {
                $key = 'CLINICALVAULE';
            }
            if (!isset($available[$key])) {
                throw new \RuntimeException("Column $column does not exist in $table");
            }
            $quoted[] = \Yii::$app->db->quoteColumnName($available[$key]);
        }
    ...
    protected function loaddata($txtfile, $table, $zip_file_name, $stat = 1) {
        ...
        try {
            ...
            //raw
            $columns = $this->getLoadColumnList($txtfile, $table);
            ...
            if ($stat == 1) {
                //tmp                        
                $tmpTable = "dhdc_tmp_$table";
                $columns = $this->getLoadColumnList($txtfile, $tmpTable);
                $sql = "LOAD DATA LOCAL INFILE " . \Yii::$app->db->quoteValue($txtfile);
                $sql.= " REPLACE INTO TABLE " . \Yii::$app->db->quoteTableName($tmpTable);
                $sql.= " CHARACTER SET UTF8";
                $sql.= " FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\r\n' IGNORE 1 LINES";
                $sql.= " ($columns) SET NOTE1=" . \Yii::$app->db->quoteValue($zip_file_name) . ",NOTE2=NOW()";
            ...
            try {
                ...
                \Yii::error($rollbackException->getMessage(), __METHOD__);
            }
            throw $e;
        }
    }

    protected function data_correct($file) {
        $ff = strtoupper(basename($file));
        if ($ff <> 'DATA_CORRECT.TXT') {
            return;
        }
        $handle = fopen($file, 'r');
        ...
        while (!feof($handle)) {
            ...
            $table='';
            if (!empty($csv[2]) && strtoupper(strtoupper($csv[2])) <> 'DATA_CORRECT') {
                $table = $this->validateTableName($csv[1]);
                $data = $csv[2];
                $json = str_replace("'", "\"", $data);
                $arr = json_decode($json, TRUE);               
                unset($arr['EDITOR_CID']);
                unset($arr['editor_cid']);
                ...
                foreach ($arr as $key => $value) {
                    if (!preg_match('/^[A-Za-z0-9_]+$/', (string)$key) || !isset($schema->columns[$key])) {
                        throw new \RuntimeException("Invalid DATA_CORRECT column $key");
                    }
                    $param = ':p' . $i++;
                    $conditions[] = \Yii::$app->db->quoteColumnName($key) . " = $param";
                    $params[$param] = $value;
                }
                if (empty($conditions)) {
                    continue;
                }
                $sql = "DELETE FROM " . \Yii::$app->db->quoteTableName($table) . " WHERE " . implode(' AND ', $conditions);
                ...
                //    }
                //}
				//$check = \Yii::$app->db->createCommand($sqlC)->count();
				//
				//if($check > 0) {
					$res = \Yii::$app->db->createCommand($sql, $params)->execute();
					echo "data_correct effect = $res record   \r\n";
				//} 
    ...
    public function actionTestCorrect() {
        $file = \Yii::getAlias('@webroot') . "/unzip/DATA_CORRECT.txt";
        $this->data_correct($file);
    }
