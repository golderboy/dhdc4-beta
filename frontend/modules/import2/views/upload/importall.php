...
<?php
$action_route = Url::to(['/import2/ajax/import-all']);

$date = date('Ymd');
$time = date('His');
$csrfParam = Json::htmlEncode(Yii::$app->request->csrfParam);
$csrfToken = Json::htmlEncode(Yii::$app->request->csrfToken);

$script = <<< JS
   
    function excec(fname) {      
        var data = {fortythree: fname, upload_date: "$date", upload_time: "$time"};
