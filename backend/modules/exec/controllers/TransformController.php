...
class TransformController extends Controller {
    ...
    public function actionSetup() {
        ...
        $this->transform_init();
        return 'เตรียมการประมวลผลเรียบร้อยแล้ว';
    }
  ...
  PRIMARY KEY (`id`)
        ...
        $this->exec_sql($sql);
        $sql = "INSERT INTO sys_transform_all (t_name,t_sql,active,bycase,version) (SELECT t.t_name,t.t_sql,t.active,t.bycase,t.version FROM sys_transform t  ORDER BY t.id);";
        $this->exec_sql($sql);
        $sql = "INSERT INTO sys_transform_all (t_name,t_sql,active,bycase,version) (SELECT t.t_name,t.t_sql,t.active,t.bycase,t.version FROM sys_transform_plus t  ORDER BY t.id);";
        $this->exec_sql($sql);
...
ORDER BY t.id ASC ";
        ...
        foreach ($raw as $value) {
            ...
            $t_name = $value['t_name'];
            if ($t_sql && $t_name) {
                if (!preg_match('/^[A-Za-z0-9_]+$/', (string)$t_name)) {
