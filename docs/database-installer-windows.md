...
- ชุดโปรแกรม DHDC4 รุ่น `v4.0.0`
- ชุดฐานข้อมูล `dhdc4-database-installer-v4.0.1.zip` และไฟล์ `.zip.sha256`
- Apache
...
C:\DHDC4-Install\dhdc4-database-installer-v4.0.1
```
...
Get-FileHash -Algorithm SHA256 '.\dhdc4-database-installer-v4.0.1.zip'
Get-Content '.\dhdc4-database-installer-v4.0.1.zip.sha256'
```

ค่า SHA-256 สองรายการต้องตรงกันทุกตัว จากนั้นแตกไฟล์:

```powershell
Expand-Archive '.\dhdc4-database-installer-v4.0.1.zip' -DestinationPath '.\database' -Force
Set-Location '.\database\dhdc4-database-installer-v4.0.1'
```
...
```powershell
& 'C:\xampp\mysql\bin\mariadb.exe' --host=localhost --port=3306 --user=root --password
```
...
```powershell
$env:DHDC_DB_DSN = 'mysql:host=localhost;dbname=dhdc4;port=3306'
$env:DHDC_DB_USER = 'dhdc4'
$env:DHDC_DB_PASSWORD = 'ใส่รหัสผ่านฐานข้อมูลจริง'
